...
check_diatom_names <- function(diatnames="", 
  ...
  conversion_df$taxon_name_updated <- lookup_e(conversion_df$taxon_name_synonym_authority, diat_omnidia[,c("DENOM","DENOM_lev1")])
  
  #replace empty most synonym column with original user name
  conversion_df[is.na(conversion_df$most_recent_omnidia_code)==TRUE,]$taxon_name_updated <- conversion_df[is.na(conversion_df$most_recent_omnidia_code)==TRUE,]$user_taxa_lev1
  ...
  flagged <- conversion_df %>%
    ...
    mutate(
      ...
      # Boolean flag
      is_mismatch = length(all_codes) > 1,
      
      # Detailed description accounting for multiple synonyms
      mismatch_desc = if (!is_mismatch) {
        "No synonyms found"
      } else if (length(syns_all) >= 1) {
        paste0("Multiple synonyms: ", length(all_codes), " distinct codes found. OTHER_SYNONYMS_CODES contains ", length(syns_all), " synonyms (", paste(syns_all, collapse = ", "), ").")
      } 
      else {
        #paste0("Mismatch: CODE (", Omnidia_code, ") and MOST_RECENT_SYN (", most_recent_omnidia_code, ") differ.")
      }
    ) %>%
  ...
  names(conversion_df)[c(10,11)] <- c("multiple_synonyms","synonym_mismatch_notes")
